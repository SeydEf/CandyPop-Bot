...
async def admin_stats_menu(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    text = (
        ...
        f"💰 <b>تراکنش‌ها و آمار مالی جامع:</b>\n"
        f"  • کل فاکتورهای صادرشده: <b>{to_persian_digits(stats['total_invoices_count'])}</b> فقره ({format_price(stats['total_invoices_amount'])})\n"
        f"  • 🟢 <b>پرداخت‌های موفق (درآمد کل):</b> <b>{to_persian_digits(stats['paid_invoices_count'])}</b> فقره ({format_price(stats['paid_revenue'])})\n"
        f"      ├ 🛒 فروش/تمدید مستقیم اشتراک: <b>{to_persian_digits(stats['subs_sales_count'])}</b> فقره ({format_price(stats['subs_sales_revenue'])})\n"
        f"      ├ 💳 شارژ کیف پول: <b>{to_persian_digits(stats['topups_count'])}</b> فقره ({format_price(stats['topups_revenue'])})\n"
        f"      ├ 💳 پرداختی کارت به کارت: <b>{to_persian_digits(stats.get('card_count', 0))}</b> فقره ({format_price(stats.get('card_revenue', 0))})\n"
        f"      └ 👛 پرداختی از کیف پول: <b>{to_persian_digits(stats.get('wallet_count', 0))}</b> فقره ({format_price(stats.get('wallet_revenue', 0))})\n"
        f"  • 🟡 <b>در انتظار بررسی (پندینگ):</b> <b>{to_persian_digits(stats['pending_invoices_count'])}</b> فقره ({format_price(stats['pending_amount'])})\n"
        f"  • 🔴 <b>فاکتورهای ردشده:</b> <b>{to_persian_digits(stats['rejected_invoices_count'])}</b> فقره ({format_price(stats['rejected_amount'])})\n"
        f"  • 💼 <b>مجموع موجودی کیف پول کاربران:</b> <b>{format_price(stats['total_wallets_balance'])}</b>\n\n"
