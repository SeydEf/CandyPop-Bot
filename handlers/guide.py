...
def build_os_menu_text() -> str:
    return (
        ...
        "سیستم‌عامل دستگاه خود را از گزینه‌های زیر انتخاب کنید تا فهرست نرم‌افزارهای مناسب و راهنمای راه‌اندازی نمایش داده شود:"
    )
...
def build_app_list_text(os_key: str) -> str:
    ...
    rec_str = (
        f"⭐️ <b>برنامه‌های پیشنهادی:</b> {' و '.join(rec_names)}\n\n"
        if rec_names
        else ""
    )
    return (
        ...
        f"{description}\n\n"
        f"{rec_str}"
        "👇 <i>برای مشاهده آموزش گام‌به‌گام و دریافت آخرین نسخه، برنامه مورد نظر را انتخاب کنید:</i>"
    )
