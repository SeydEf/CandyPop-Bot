...
async def get_user_financial_summary(tg_id: int) -> dict[str, Any]:
    db = await get_db()
    async with db.execute(
        ...
        if row:
            return {
                ...
                "topups_amount": row[2] or 0,
                "subs_amount": row[3] or 0,
            }
...
PERMISSION_TITLES: dict[str, str] = {
    ...
    "send_user_message": "ارسال پیام مستقیم به کاربران",
    ...
    "reapprove_invoices": "بازبینی و تأیید مجدد فاکتورهای ردشده",
...
async def increment_discount_usage(code: str, tg_id: int | None = None) -> None:
    clean_code = code.strip().upper()
    db = await get_db()
    await db.execute(
